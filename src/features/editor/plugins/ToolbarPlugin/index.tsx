...
import { IS_APPLE } from "@/features/editor/utils/environment";
import {
...
import { getSelectedNode } from "@/features/editor/utils/getSelectedNode";
...
import { GeminiDialog } from "../GeminiPlugin";
...
export default function ToolbarPlugin({
  ...
  return (
    <div className="toolbar">
      {/* <button className="toolbar-item spaced" onClick={()=>{
        activeEditor.update(()=>{
          const selection=$getSelection();
          if($isRangeSelection(selection)){
            const anchorNode=selection.anchor.getNode();
            ...
            const textNode=$createTextNode("后面");
            textNode.setStyle("background-color:red")
      ...
      <button className="toolbar-item spaced" onClick={()=>{
        activeEditor.update(()=>{
          ...
          if($isRangeSelection(selection)){
            const anchorNode=selection.anchor.getNode();
            const newNode=$createParagraphNode()
            const textNode=$createTextNode("前面");
            textNode.setStyle("background-color:blue")
            newNode.append(textNode);
      ...
      <button
        onClick={() => {
          showModal("Gemini AI 助手", (onClose) => (
            <GeminiDialog activeEditor={activeEditor} onClose={onClose} />
          ));
        }}
        className="toolbar-item spaced"
        title="Gemini AI 助手"
      >
        <i className="format gemini" />
      </button>
      <button
        onClick={() => {
